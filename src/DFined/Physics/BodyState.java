...
public class BodyState {
    ...
    public void calculateInfluence(SolarSystemState bodyStates, List<BodyState> others) {
        for (BodyState other : others) {
            Vector3D radius = position.subtract(other.position);
            double collDist = other.getBody().getRadius()/Physics.DISTANCE_SCALE + this.getBody().getRadius()/Physics.DISTANCE_SCALE;
            ...
            addAcceleration(accel, other.getBody(), radius.getNorm());
        }
    }

    public void influenceOther(Vector3D accel, BodyState other, double rad) {
        other.addAcceleration(accel, this.getBody(), rad);
    }
